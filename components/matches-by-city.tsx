...
export default function MatchesByCity({ matches }: MatchesByCityProps) {
  ...
  useEffect(() => {
    const generateSampleData = (): Match[] => {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        const season =
          seasonsArray[Math.floor(Math.random() * seasonsArray.length)];
        ...
        sampleMatches.push({
          ...
          team2,
          winner,
          date: `2024-0${Math.floor(Math.random() * 9) + 1}-${String(
        ...
        <div class="flex items-center justify-center h-full bg-gray-50 p-4">
          <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm max-w-md w-full text-center border">
            ...
            <div class="text-xs sm:text-sm text-gray-600 space-y-2">
              <p>Map visualization shows team wins across ${
  ...
  useEffect(() => {
    if (!teamMapRef.current || !matchesData.length || loading || !mapboxLoaded)
      return;

    const filtered: Match[] =
      season === "All"
        ? matchesData
        : matchesData.filter((m) => m.season === season);
  ...
  <div style="padding: 12px; max-height: 300px;">
    ...
    <div style="max-height: 200px; overflow-y: auto; padding: 0 10px; white-space: normal; overflow-wrap: break-word;">
      ${teamsList}
    </div>
...
`);
  ...
  const uniqueCities: number = new Set(
    filteredMatches.map((m) => m.city).filter(Boolean)
  ).size;
  const uniqueTeams: number = new Set(
    [
      ...filteredMatches.map((m) => m.team1),
      ...filteredMatches.map((m) => m.team2),
    ].filter(Boolean)
  ).size;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600 mt-1">
            Interactive analysis of IPL matches across cities
          </p>
        </div>
        ...
        <Card className="xl:col-span-2 h-[350px] sm:h-[450px] lg:h-[500px]">
          ...
          <CardContent className="p-2 sm:p-4">
            {loading ? (
              ...
              <div
                ref={teamMapContainerRef}
                className="h-[250px] sm:h-[350px] lg:h-[400px] w-full rounded-lg border"
              />
            )}
          </CardContent>
        </Card>
